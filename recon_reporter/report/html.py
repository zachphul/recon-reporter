...
.kev{display:inline-block;background:#e0533d;color:#0b0c0f;font-weight:700;font-size:11px;
padding:1px 6px;border-radius:4px;margin-left:4px}
.epss{display:inline-block;background:#2a2f3a;color:#e0c24d;font-size:11px;padding:1px 6px;
...
def _cve_cell(cves: list) -> str:
    """Render a service's CVEs with KEV (actively-exploited) and EPSS badges."""
    if not cves:
        return "—"
    parts = []
    for c in cves:
        s = _e(c.id)
        if c.kev:
            title = "ransomware-linked" if c.ransomware else "actively exploited (CISA KEV)"
            s += f"<span class='kev' title='{title}'>KEV</span>"
        if c.epss is not None:
            s += f"<span class='epss' title='EPSS: exploitation probability'>{c.epss:.0%}</span>"
        parts.append(s)
...
def render(scan: ScanRun, analysis: Analysis | None) -> str:
    ...
    p.append("<h1>Security Assessment</h1>")
    ...
    for h in scan.hosts:
        ...
        for svc in h.services:
            p.append(f"<tr><td class='mono'>{svc.port}/{_e(svc.protocol)}</td><td>{_e(svc.service or '?')}</td>"
                     f"<td>{_e(svc.label or '-')}</td><td>{_cve_cell(svc.cves)}</td></tr>")
        p.append("</table>")

    p.append(f"<footer>Generated by ReconLens on {_e(datetime.now().isoformat(timespec='seconds'))}.</footer>")
